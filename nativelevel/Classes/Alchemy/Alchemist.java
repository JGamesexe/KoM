...
public class Alchemist {
    ...
    public static final Jobs.Classe classe = Jobs.Classe.Alquimista;
    public static final String name = "Alquimista";
    ...
    public static void onHit(EntityDamageByEntityEvent ev) {
        ...
        if (weapon.getType().toString().contains("HOE")) {
            ...
            if ((((Jobs.rnd.nextInt(101) - (attacker.getLevel() / 5)))) < 50) return;
    ...
    public static void tossTnt(PlayerInteractEvent ev) {
        ...
        if (ev.getPlayer().getInventory().getItemInMainHand().getType() == Material.TNT) {
            ...
            TNTPrimed tnt = ev.getPlayer().getWorld().spawn(ev.getPlayer().getLocation(), TNTPrimed.class);
            tnt.getLocation().add(0.5D, 0.5D, 0.5D);

            EntityLiving nmsEntityLiving = (((CraftLivingEntity) ev.getPlayer()).getHandle());
...
// !=- SKILLS AREA -=! - !=- SKILLS AREA -=! - !=- SKILLS AREA -=! - !=- SKILLS AREA -=! - !=- SKILLS AREA -=! - !=- SKILLS AREA -=! - !=- SKILLS AREA -=! - !=- SKILLS AREA -=! - !=- SKILLS AREA -=!
    ...
    public static final List<Skill> skillList = Arrays.asList(
            new Skill(classe, "Criar Poções", 2, false, new String[]{"§9Aumenta chances de sucesso ao criar poções.", "§9Quanto maior seu nivel, maior a chance"}, true),
            new Skill(classe, "Foice Mestra", 4, true, new String[]{"§9Bonus ao bater com foices."}),
            new Skill(classe, "Sangue Tóxico", 5, false, new String[]{"§9Bonus ao bater com foices."}),
            new Skill(classe, "Usar Poções de Splash", 15, true,new String[]{"§9Pode arremeçar Poções"}),
            new Skill(classe, "Arremeçar TNT", 20, true, new String[]{"§9Não sente efeitos de veneno."}),
            new Skill(classe, "Caldeirão de Poções", 30, true, new String[]{"§9Pode misturar poções no caldeirão"})
    );

    public enum Skills {
        Criar_Pocoes(skillList.get(0)),
        Foice_Mestra(skillList.get(1)),
        Sangue_Toxico(skillList.get(2)),
        Usar_Pocoes_de_Splash(skillList.get(3)),
        Arremecar_TNT(skillList.get(4)),
        Caldeirao_de_Pocoes(skillList.get(5));
