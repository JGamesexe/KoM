...
//    }
    ...
    public void spawn(CreatureSpawnEvent ev) {
        ...
        if (ev.isCancelled() || entity == null) KoM.debug("spawn Cancelado #-1");
        if (ev.isCancelled()) return;

        if (ev.getSpawnReason().equals(SpawnReason.CUSTOM)) return;

        if (entity.hasMetadata("NPC")) return;
        if (entity.hasMetadata("shopkeeper")) return;

        if (ev.getSpawnReason().equals(SpawnReason.NATURAL)) {
            double q = 0;

            if (entity instanceof Monster) {
                for (Entity e : ev.getEntity().getNearbyEntities(20, 32, 20))
                    if (e instanceof LivingEntity && e instanceof Monster)
                        q++;
            } else {
                for (Entity e : ev.getEntity().getNearbyEntities(20, 32, 20))
                    if (e instanceof LivingEntity && !(e instanceof Player || e instanceof Monster))
                        q += 2.5;
            }

            if (q >= 7) ev.setCancelled(true);
            if (ev.isCancelled()) return;

        }

        if (entity.getWorld().getName().equalsIgnoreCase(CFG.mundoGuilda)) {
            spawnGuilda(ev);
        } else if (entity.getWorld().getName().equalsIgnoreCase(CFG.mundoDungeon)) {
            spawnDungeon(ev);
        } else {
            ev.setCancelled(true);
            KoM.debug("spawn Cancelando #2 !" + entity.getWorld().getName());
        }
        ...
        if (ev.getEntity().getCustomName() == null)
            Bukkit.getScheduler().runTaskLater(KoM._instance, () -> {
                if (entity instanceof Monster) {
                    ...
                    entity.setCustomName("§7" + LangMinecraft.get().getMobName(entity.getType(), true) + " §8[" + zona + "§8]");
                    MetaShit.setMetaObject("mobLevel", entity, zona);
                } else {
                    entity.setCustomName("§7" + LangMinecraft.get().getMobName(entity.getType(), false));
                }
            }, 5);

        if (!ev.isCancelled()) KoM.debug("Spawando " + entity.getType() + " em " + LocUtils.loc2str(entity.getLocation()) + "(" + ev.getSpawnReason() + ")");

    }

    private void spawnGuilda(CreatureSpawnEvent ev) {
        Entity entity = ev.getEntity();

        if (entity instanceof Monster) {

            if (ClanLand.isSafeZone(entity.getLocation())) ev.setCancelled(true);
            if (ev.isCancelled()) return;

            if (ev.getLocation().getY() >= (GeneralUtils.getHighestBlockAt(ev.getLocation()).getY() - 2))
                if (Jobs.rnd.nextInt(3333) == 666) {
                    int zona = ClanLand.getMobLevel(entity.getLocation());
    ...
    private void spawnDungeon(CreatureSpawnEvent ev) {

        if (ev.getSpawnReason().equals(SpawnReason.NATURAL) || ev.getSpawnReason().equals(SpawnReason.JOCKEY) || ev.getSpawnReason().equals(SpawnReason.MOUNT))
            ev.setCancelled(true);
        if (ev.isCancelled()) return;

        if (!ev.getSpawnReason().equals(SpawnReason.CUSTOM)) {
            ApplicableRegionSet set = KoM.worldGuard.getRegionManager(ev.getLocation().getWorld()).getApplicableRegions(ev.getLocation());
            if (!(set.size() > 0)) {
                ev.setCancelled(true);
            }
        }
...
//    }
    ...
    public static void morreMob(EntityDeathEvent ev) {
        ...
        if (ev.getEntity().hasMetadata("boss")) {
            morreBoss(ev);
            return;
        }

        if (ev.getEntity().hasMetadata("mobLevel")) {
            if (ev.getEntity().getKiller() == null) return;

            Player player = ev.getEntity().getKiller();

            int mobLevel = (int) MetaShit.getMetaObject("mobLevel", ev.getEntity());
            if (mobLevel == 666) mobLevel = 100;

            ClanPlayer clanPlayer = ClanLand.manager.getClanPlayer(player);
            int diferenca;
            double exp;

            diferenca = mobLevel - player.getLevel();

            if (diferenca > 10) {
                player.sendMessage("O nível do Mob é muito alto para ele dar EXP");
                return;
            } else if (diferenca < 0) {
                exp = XP.getExpPorAcao(mobLevel);
            } else {
                exp = XP.getExpPorAcao(player.getLevel());
            }

            exp *= 0.055;
            XP.bruteChangeExp(player, exp, true);

            //Karma + Fama, apenas pra quem matou
            Karma.manoloMata(player, ev.getEntity());

            if (clanPlayer == null) return;

            ArrayList<Player> beneficiados = new ArrayList<>();

            for (Entity nearbyEntity : ev.getEntity().getNearbyEntities(17, 4, 17)) {
                if (nearbyEntity instanceof Player) {
                    Player nearbyPlayer = (Player) nearbyEntity;
                    ClanPlayer nearbyClanPlayer = ClanLand.manager.getClanPlayer(nearbyPlayer);
                    if (nearbyClanPlayer != null && clanPlayer.getTag().equalsIgnoreCase(nearbyClanPlayer.getTag()))
                        beneficiados.add(nearbyPlayer);
                }
            }

            for (Player playerBeneficiado : beneficiados) {
                if (!playerBeneficiado.getUniqueId().equals(player.getUniqueId())) {
                    diferenca = (mobLevel - playerBeneficiado.getLevel());

                    if (diferenca > 10) {
                        player.sendMessage("§cO nível do Mob é muito alto para ele dar EXP");
                        continue;
                    } else if (diferenca < 0) {
                        exp = XP.getExpPorAcao(mobLevel);
                    } else {
                        exp = XP.getExpPorAcao(playerBeneficiado.getLevel());
                    }

                    exp *= 0.15;

                    XP.bruteChangeExp(playerBeneficiado, exp, true);
                }
            }
    ...
    private static void morreBoss(EntityDeathEvent ev) {
        DamageListener.KoMBoss boss = DamageListener.getBoss(ev.getEntity().getUniqueId());
        for (Map.Entry<UUID, Double> dano : boss.getDanos()) {
            Player player = Bukkit.getPlayer(dano.getKey());
            if (player != null) {

                double porc = (dano.getValue() / boss.getEntity().getAttribute(Attribute.GENERIC_MAX_HEALTH).getValue());
                KoM.announce(player.getName() + " deu " + ((int) (porc * 100)) + "% de dano no BOSS");

                double exp;
                int diferenca = boss.getMobLevel() - player.getLevel();

                if (diferenca > 5) {
                    player.sendMessage("§cO nível do BOSS é muito alto para você ganhar XP.");
                    return;
                } else if (diferenca < 0) {
                    exp = (XP.getExpProximoNivel(boss.getMobLevel()) * 0.10);
                    player.sendMessage("§eVocê recebeu EXP do nível " + boss.getMobLevel() + " pela diferença de nível com o BOSS");
                } else {
                    exp = (XP.getExpProximoNivel(player.getLevel()) * 0.10);
                }

                exp *= porc;

                XP.changeExp(player, exp, true, false);
            }
        }
        DamageListener.bosses.remove(DamageListener.getBoss(ev.getEntity().getUniqueId()));
    }

//    public static void morreMob(EntityDeathEvent ev) {
//
//        if (ev.getEntity().hasMetadata("boss")) {
//            for (Map.Entry<UUID, Double> dano : DamageListener.getBoss(ev.getEntity().getUniqueId()).getDanos()) {
//                KoM.announce(dano.getKey() + " deu " + dano.getValue() + "de dano no boss");
//            }
//            DamageListener.bosses.remove(DamageListener.getBoss(ev.getEntity().getUniqueId()));
//        }
//
...
//    }
    ...
    public static void target(EntityTargetEvent ev) {
        if (ev.getTarget() != null) {
            if (ev.getTarget().getType() == EntityType.PLAYER) {
                if (CapaInvisvel.ativos.contains(ev.getTarget().getUniqueId())) ev.setCancelled(true);
            } else if (ev.getTarget().getType() == EntityType.IRON_GOLEM) {
                ev.setCancelled(true);
            }
        }
    ...
    @EventHandler
    public void enterVehicle(VehicleEnterEvent ev) {
        if (ev.getEntered() instanceof ArmorStand ||
                ev.getEntered().hasMetadata("NPC") ||
                ev.getEntered().hasMetadata("shopkeeper") ||
                KoM.mm.getAPIHelper().isMythicMob(ev.getEntered().getUniqueId()) ||
                ev.getEntered().hasMetadata("boss")) {
    ...
    @EventHandler
    public void mobSpawnSpawner(SpawnerSpawnEvent event) {

        Entity entity = event.getEntity();
        Location loc = event.getSpawner().getLocation();

        if (loc.add(0, -3, 0).getBlock().getType().equals(Material.BEDROCK)) {
        ...
        } else if (loc.getWorld().getName().equalsIgnoreCase(CFG.mundoGuilda)) {
            event.setCancelled(true);
            event.getSpawner().setType(Material.AIR);
        }
    }
    ...
    private void attEntityEquip(Entity entity, Chest chest) {
        ...
        ItemStack maoPrimaria = chest.getBlockInventory().getItem(23);
        ItemStack maoSecundaria = chest.getBlockInventory().getItem(24);
